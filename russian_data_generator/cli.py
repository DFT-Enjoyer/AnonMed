...
from .generators.age import generate_age, generate_age_raw, verbalize_age
from .generators.birth_certificate import (
    generate_birth_certificate,
    generate_birth_certificate_raw,
    verbalize_birth_certificate,
)
from .generators.birthdate import generate_birthdate, generate_birthdate_raw, verbalize_birthdate
from .generators.driver_license import (
    generate_driver_license,
    generate_driver_license_raw,
    verbalize_driver_license,
)
from .generators.inn import generate_inn, generate_inn_raw, verbalize_inn
from .generators.mse import generate_mse, generate_mse_raw, verbalize_mse
from .generators.oms import generate_oms, generate_oms_raw, verbalize_oms
from .generators.passport import generate_passport, generate_passport_raw, verbalize_passport
...
from .generators.snils import generate_snils, generate_snils_raw, verbalize_snils
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--raw",
        action="store_true",
        help="Выводить данные в цифровом (raw) формате вместо устной вербализации",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    for _ in range(args.count):
        if args.raw:
            # Генерация и вывод сырых данных
            if args.type == "phone_mobile":
                print(generate_mobile_phone_raw())
            elif args.type == "phone_landline":
                print(generate_landline_phone_raw())
            elif args.type == "snils":
                print(generate_snils_raw())
            elif args.type == "passport":
                print(generate_passport_raw())
            elif args.type == "birthdate":
                print(generate_birthdate_raw())
            elif args.type == "inn":
                print(generate_inn_raw())
            elif args.type == "oms":
                print(generate_oms_raw())
            elif args.type == "age":
                print(generate_age_raw())
            elif args.type == "mse":
                print(generate_mse_raw())
            elif args.type == "birth_certificate":
                print(generate_birth_certificate_raw())
            elif args.type == "driver_license":
                print(generate_driver_license_raw())
            else:
                raise ValueError(f"Неизвестный тип: {args.type}")
        else:
            # Стандартный вывод вербализации
            if args.type == "phone_mobile":
